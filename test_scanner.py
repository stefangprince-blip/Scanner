...
print("\n--- catalyst scoring ---")
def s(h, b="", w=1.0): return scoring.score(h, b, w)
...
r3 = s("Company Announces Record Revenue, Raises Guidance and Pricing of Public Offering")
check("dilution damps score", r3["score"] < r2["score"], f"{r3['score']} vs {r2['score']}")
...
check("dollar size lifts score", big["score"] > small["score"], f"{big['score']} vs {small['score']}")

stuffed = s("Company Announces Launch, Collaboration, Expansion, Milestone and Partnership")
check("keyword stuffing capped below real catalyst",
      stuffed["score"] < s("Company Announces FDA Approval")["score"],
      f"{stuffed['score']}")
check("score never exceeds 100", s("FDA Approval and Definitive Merger Agreement for $900 Million and Met Primary Endpoint")["score"] <= 100)

print("\n--- store TTL + dedup ---")
import os
if os.path.exists("t.db"): os.remove("t.db")
st = store.Store("t.db")
a1 = {"ticker":"ABCD","headline":"ABCD Announces FDA Approval","score":50,"tags":["FDA approval"],"dilution":[],"distress":[],"source":"X","published":time.time()}
...
scn = sc.Scanner(quote_provider=quotes.NullProvider(), store=store.Store("t2.db") if not os.path.exists("t2.db") else store.Store("t2.db"))
scn.seed_demo()
rows = scn.rows()
check("demo seeded rows", len(rows) >= 5, str(len(rows)))
check("rows sorted by heat desc", all(rows[i]["heat"] >= rows[i+1]["heat"] for i in range(len(rows)-1)))
tick_map = {r["ticker"]: r for r in rows}
check("AUVI M&A present", "AUVI" in tick_map)
check("SIEB offering flagged", tick_map.get("SIEB",{}).get("dilution"), str(tick_map.get("SIEB",{}).get("dilution")))
